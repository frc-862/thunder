...
public class PathPlannerTrajectory extends Trajectory {
    ...
    protected PathPlannerTrajectory(List<Waypoint> pathPoints, List<EventMarker> markers,
            PathConstraints constraints, boolean reversed, boolean fromGUI) {
        super(generatePath(pathPoints, constraints.maxVelocity, constraints.maxAcceleration,
                reversed));
    ...
    private PathPlannerTrajectory(List<State> states, List<EventMarker> markers,
            StopEvent startStopEvent, StopEvent endStopEvent, boolean fromGUI) {
        super(states);
    ...
    public static PathPlannerTrajectory transformTrajectoryForAlliance(
        ...
        if (alliance == DriverStation.Alliance.Red) {
            ...
            return new PathPlannerTrajectory(transformedStates, trajectory.markers,
                    trajectory.startStopEvent, trajectory.endStopEvent, trajectory.fromGUI);
        } else {
            return trajectory;
        }
    }
    ...
    private static List<State> generatePath(List<Waypoint> pathPoints, double maxVel,
    ...
    private static void calculateMaxVel(List<PathPlannerState> states, double maxVel,
            double maxAccel, boolean reversed) {
    ...
    private static void calculateVelocity(List<PathPlannerState> states, List<Waypoint> pathPoints,
            double maxAccel) {
    ...
    private static void recalculateValues(List<PathPlannerState> states, boolean reversed) {
        for (int i = states.size() - 1; i >= 0; i--) {
            ...
            if (i != states.size() - 1) {
                ...
                now.angularVelocityRadPerSec =
                        MathUtil.inputModulus(
                                ...
                                -Math.PI, Math.PI) / dt;
                now.holonomicAngularVelocityRadPerSec = MathUtil.inputModulus(
                        next.holonomicRotation.getRadians() - now.holonomicRotation.getRadians(),
                        -Math.PI, Math.PI) / dt;
            }

            if (Double.isInfinite(now.curveRadius) || Double.isNaN(now.curveRadius)
                    || now.curveRadius == 0) {
            ...
            if (reversed) {
                ...
                now.poseMeters =
                        new Pose2d(now.poseMeters.getTranslation(), Rotation2d.fromDegrees(h));
            }
        }
    }

    private static List<PathPlannerState> joinSplines(List<Waypoint> pathPoints, double maxVel,
            double step) {
        ...
        for (int i = 0; i < numSplines; i++) {
            ...
            for (double t = 0; t <= endStep; t += step) {
                Translation2d p = GeometryUtil.cubicLerp(startPoint.anchorPoint,
                        startPoint.nextControl, endPoint.prevControl, endPoint.anchorPoint, t);
                ...
                double holonomicRot = GeometryUtil.cosineInterpolate(startRot,
                        Rotation2d.fromDegrees(startRot.getDegrees() + deltaRot),
                        ((i + t) - startRotIndex) / rotRange).getDegrees();
                ...
                if (i > 0 || t > 0) {
                    ...
                    double hypot = s1.poseMeters.getTranslation()
                            .getDistance(s2.poseMeters.getTranslation());
                    state.deltaPos = hypot;

                    double heading =
                            Math.toDegrees(Math.atan2(s1.poseMeters.getY() - s2.poseMeters.getY(),
                                    s1.poseMeters.getX() - s2.poseMeters.getX())) + 180;
                    heading = MathUtil.inputModulus(heading, -180, 180);
                    state.poseMeters = new Pose2d(state.poseMeters.getTranslation(),
                            Rotation2d.fromDegrees(heading));

                    if (i == 0 && t == step) {
                        states.get(states.size() - 1).poseMeters = new Pose2d(
                ...
                if (state.velocityMetersPerSecond == -1)
                    state.velocityMetersPerSecond = maxVel;
    ...
    private static double calculateRadius(PathPlannerState s0, PathPlannerState s1,
            PathPlannerState s2) {
    ...
    private void calculateMarkerTimes(List<Waypoint> waypoints) {
        for (EventMarker marker : this.markers) {
            ...
            marker.positionMeters = GeometryUtil.cubicLerp(startPoint.anchorPoint,
                    startPoint.nextControl, endPoint.prevControl, endPoint.anchorPoint, t);

            int statesPerWaypoint = (int) (1 / PathPlanner.resolution);
            startIndex = (int) ((statesPerWaypoint * marker.waypointRelativePos)
    ...
    public static class PathPlannerState extends State {
        ...
        private PathPlannerState interpolate(PathPlannerState endVal, double t) {
            ...
            lerpedState.velocityMetersPerSecond = GeometryUtil.doubleLerp(velocityMetersPerSecond,
                    endVal.velocityMetersPerSecond, t);
            lerpedState.accelerationMetersPerSecondSq = GeometryUtil.doubleLerp(
                    accelerationMetersPerSecondSq, endVal.accelerationMetersPerSecondSq, t);
            Translation2d newTrans = GeometryUtil.translationLerp(poseMeters.getTranslation(),
                    endVal.poseMeters.getTranslation(), t);
            Rotation2d newHeading = GeometryUtil.rotationLerp(poseMeters.getRotation(),
                    endVal.poseMeters.getRotation(), t);
            lerpedState.poseMeters = new Pose2d(newTrans, newHeading);
            lerpedState.angularVelocityRadPerSec = GeometryUtil.doubleLerp(angularVelocityRadPerSec,
                    endVal.angularVelocityRadPerSec, t);
            lerpedState.holonomicAngularVelocityRadPerSec = GeometryUtil.doubleLerp(
    ...
    protected static class Waypoint {
        ...
        protected Waypoint(Translation2d anchorPoint, Translation2d prevControl,
                Translation2d nextControl, double velOverride, Rotation2d holonomicRotation,
                boolean isReversal, boolean isStopPoint, StopEvent stopEvent) {
    ...
    public static class StopEvent {
        public enum ExecutionBehavior {
            PARALLEL("parallel"), SEQUENTIAL("sequential"), PARALLEL_DEADLINE("parallelDeadline");
        ...
        public enum WaitBehavior {
            NONE("none"), BEFORE("before"), AFTER("after"), DEADLINE("deadline"), MINIMUM(
                    "minimum");
        ...
        public StopEvent(List<String> names, ExecutionBehavior executionBehavior,
                WaitBehavior waitBehavior, double waitTime) {
            this.names = names;
