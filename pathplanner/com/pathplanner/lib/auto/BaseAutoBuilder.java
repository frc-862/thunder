package frc.thunder.pathplanner.com.pathplanner.lib.auto;
...
public abstract class BaseAutoBuilder {
     ...
     *        typically be called once ath the beginning of an auto.
     ...
     *        of the field.
     */
    protected BaseAutoBuilder(Supplier<Pose2d> poseSupplier, Consumer<Pose2d> resetPose,
            Map<String, Command> eventMap, DrivetrainType drivetrainType,
     ...
     *        of the field.
     */
    protected BaseAutoBuilder(Supplier<Pose2d> poseSupplier, Map<String, Command> eventMap,
            DrivetrainType drivetrainType, boolean useAllianceColor) {
    ...
    protected BaseAutoBuilder(Supplier<Pose2d> poseSupplier, Map<String, Command> eventMap,
        ...
        this(poseSupplier, (pose) -> {
        }, eventMap, drivetrainType, true);
    }

    /**
     * Create a path following command for a given trajectory. This will not trigger any events
     * while path following.
     *
     * <p>
     * Override this to create auto builders for your custom path following commands.
     *
    ...
    /**
     * Create a sequential command group that will follow each path in a path group and trigger
     * events as it goes. This will not run any stop events.
     *
     * @param pathGroup The path group to follow
     * @return Command for following all paths in the group
     */
    ...
    /**
     * Create a command that will call the resetPose consumer with the first pose of the path. This
     * is usually only used once at the beginning of auto.
     *
    ...
    protected static CommandBase wrappedEventCommand(Command eventCommand) {
        return new FunctionalCommand(eventCommand::initialize, eventCommand::execute,
                eventCommand::end, eventCommand::isFinished,
     ...
     *
     * <p>
     * Using this does have its limitations, but it should be good enough for most teams. However,
     * if you want the auto command to function in a different way, you can create your own class
     * that extends BaseAutoBuilder and override existing builder methods to create the command
     * group however you wish.
     *
