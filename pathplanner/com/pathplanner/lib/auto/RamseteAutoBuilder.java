...
import frc.thunder.pathplanner.com.pathplanner.lib.commands.PPRamseteCommand;
...
public class RamseteAutoBuilder extends BaseAutoBuilder {
     ...
     *
     * <p>
     * This auto builder will use PPRamseteCommand to follow paths.
     *
     ...
     */
    public RamseteAutoBuilder(Supplier<Pose2d> poseSupplier, Consumer<Pose2d> resetPose,
            RamseteController controller, DifferentialDriveKinematics kinematics,
            SimpleMotorFeedforward feedforward,
            Supplier<DifferentialDriveWheelSpeeds> speedsSupplier, PIDConstants driveConstants,
            BiConsumer<Double, Double> outputVolts, Map<String, Command> eventMap,
            Subsystem... driveRequirements) {
        this(poseSupplier, resetPose, controller, kinematics, feedforward, speedsSupplier,
                driveConstants, outputVolts, eventMap, false, driveRequirements);
    }
    ...
    public RamseteAutoBuilder(Supplier<Pose2d> poseSupplier, Consumer<Pose2d> resetPose,
            RamseteController controller, DifferentialDriveKinematics kinematics,
            BiConsumer<Double, Double> outputMetersPerSecond, Map<String, Command> eventMap,
            Subsystem... driveRequirements) {
        this(poseSupplier, resetPose, controller, kinematics, outputMetersPerSecond, eventMap,
                false, driveRequirements);
    }
    ...
    public CommandBase followPath(PathPlannerTrajectory trajectory) {
        if (usePID) {
            return new PPRamseteCommand(trajectory, poseSupplier, controller, feedforward,
                    kinematics, speedsSupplier, pidControllerFromConstants(driveConstants),
                    pidControllerFromConstants(driveConstants), output, useAllianceColor,
                    driveRequirements);
        } else {
            return new PPRamseteCommand(trajectory, poseSupplier, controller, kinematics, output,
