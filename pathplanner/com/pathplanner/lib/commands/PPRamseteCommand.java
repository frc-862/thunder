...
public class PPRamseteCommand extends CommandBase {
    ...
    private static BiConsumer<Translation2d, Rotation2d> logError =
            PPRamseteCommand::defaultLogError;
     ...
     * @param outputVolts A function that consumes the computed left and right outputs (in volts)
     *        for the robot drive.
     ...
     */
    public PPRamseteCommand(PathPlannerTrajectory trajectory, Supplier<Pose2d> poseSupplier,
            ...
            boolean useAllianceColor, Subsystem... requirements) {
    ...
    public PPRamseteCommand(PathPlannerTrajectory trajectory, Supplier<Pose2d> poseSupplier,
        ...
        this(trajectory, poseSupplier, controller, feedforward, kinematics, speedsSupplier,
                leftController, rightController, outputVolts, false, requirements);
    }
     ...
     * @param kinematics The kinematics for the robot drivetrain.
     * @param outputMetersPerSecond A function that consumes the computed left and right wheel
     *        speeds.
     * @param useAllianceColor Should the path states be automatically transformed based on alliance
     *        color? In order for this to work properly, you MUST create your path on the blue side
     *        of the field.
     ...
     */
    public PPRamseteCommand(PathPlannerTrajectory trajectory, Supplier<Pose2d> poseSupplier,
            RamseteController controller, DifferentialDriveKinematics kinematics,
            BiConsumer<Double, Double> outputMetersPerSecond, boolean useAllianceColor,
            Subsystem... requirements) {
        this.trajectory = trajectory;
        this.poseSupplier = poseSupplier;
        this.controller = controller;
    ...
    public PPRamseteCommand(PathPlannerTrajectory trajectory, Supplier<Pose2d> poseSupplier,
        ...
        this(trajectory, poseSupplier, controller, kinematics, outputMetersPerSecond, false,
                requirements);
    }
    ...
    public void initialize() {
        if (useAllianceColor && trajectory.fromGUI) {
            transformedTrajectory = PathPlannerTrajectory.transformTrajectoryForAlliance(trajectory,
        ...
        PathPlannerTrajectory.PathPlannerState initialState =
                transformedTrajectory.getInitialState();

        this.prevSpeeds = this.kinematics
    ...
    public void execute() {
        ...
        if (this.usePID) {
            double leftFeedforward = this.feedforward.calculate(leftSpeedSetpoint,
                    (leftSpeedSetpoint - this.prevSpeeds.leftMetersPerSecond) / dt);
            double rightFeedforward = this.feedforward.calculate(rightSpeedSetpoint,
                    (rightSpeedSetpoint - this.prevSpeeds.rightMetersPerSecond) / dt);

            leftOutput = leftFeedforward + this.leftController
                    .calculate(this.speedsSupplier.get().leftMetersPerSecond, leftSpeedSetpoint);
            rightOutput = rightFeedforward + this.rightController
                    .calculate(this.speedsSupplier.get().rightMetersPerSecond, rightSpeedSetpoint);
        } else {
    ...
    private static void defaultLogError(Translation2d translationError, Rotation2d rotationError) {
        ...
        SmartDashboard.putNumber("PPRamseteCommand/rotationErrorDegrees",
                rotationError.getDegrees());
    }
     ...
     */
    public static void setLoggingCallbacks(Consumer<PathPlannerTrajectory> logActiveTrajectory,
            Consumer<Pose2d> logTargetPose, Consumer<ChassisSpeeds> logSetpoint,
