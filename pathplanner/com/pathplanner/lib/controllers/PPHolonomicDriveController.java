package frc.thunder.pathplanner.com.pathplanner.lib.controllers;
...
public class PPHolonomicDriveController {
     ...
     */
    public PPHolonomicDriveController(PIDController xController, PIDController yController,
            PIDController rotationController, PIDController poseController) {
    ...
    public ChassisSpeeds calculate(Pose2d currentPose, PathPlannerState referenceState) {
        double xFF = referenceState.velocityMetersPerSecond
                * referenceState.poseMeters.getRotation().getCos();
        double yFF = referenceState.velocityMetersPerSecond
                * referenceState.poseMeters.getRotation().getSin();
        ...
        if (!this.isEnabled) {
            return ChassisSpeeds.fromFieldRelativeSpeeds(xFF, yFF, rotationFF,
        ...
        double rotationFeedback =
                this.rotationController.calculate(currentPose.getRotation().getRadians(),
                        referenceState.holonomicRotation.getRadians());
