package frc.thunder.pathplanner.com.pathplanner.lib.server;
...
import frc.thunder.pathplanner.org.json.simple.parser.ParseException;
...
public class PathPlannerServer {
                    ...
                    while (true) {
                        ...
                        synchronized (clients) {
                            clients.add(new PathPlannerServerThread(socket,
                                    PathPlannerServer::handleMessage));
        ...
        try {
            ...
            switch (command) {
                case "updatePath":
                    ...
                    File pathFile = new File(Filesystem.getDeployDirectory(),
                            "pathplanner/" + pathName + ".path");
    ...
    public static void sendActivePath(List<Trajectory.State> states) {
        ...
        for (int i = 0; i < states.size() - 1; i += 10) {
            ...
            stateArr.add(
                    Math.round(states.get(i).poseMeters.getTranslation().getX() * 100.0) / 100.0);
            stateArr.add(
                    Math.round(states.get(i).poseMeters.getTranslation().getY() * 100.0) / 100.0);
