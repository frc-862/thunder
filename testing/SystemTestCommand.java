 ...
 * tab called "System Tests" where users can view the current test, and
 * pass/fail
...
public class SystemTestCommand extends CommandBase {
	...
	private void resetUserIndication() {
		...
		if (successButton != null)
			successButton.setBoolean(false);
		if (failureButton != null)
			failureButton.setBoolean(false);
	}

	static void register(SystemTest test) {
		if (queue.isEmpty()) {
			queue.add(new SystemTest("Please Indicate Success to Begin Testing",
					LightningFaultCodes.getFaultCode("GENERAL"), SystemTest.Priority.DO_FIRST) {
				@Override
	 ...
	 * {@link frc.lightningUtil.LightningRobot#testInit()}.
	 */
	public SystemTestCommand() {
		...
		tab.addString("Current Test", () -> {
			...
			else if (queue.size() - passed - failed == 0)
		...
		tab.addString("User Indication", () -> {
			if (userIndication.equals(UserIndication.SUCCESS))
				return "SUCCESS";
			if (userIndication.equals(UserIndication.FAILURE))
				return "FAILURE";
			return "NONE";
		});
	...
	public void execute() {
		...
		if (isResting) {
			if (Timer.getFPGATimestamp() - restStarted < restDuration)
				return;
			isResting = false;
		}
